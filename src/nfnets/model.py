...
class NFNet_BN(nn.Module):
    def __init__(self, num_classes: int, variant: str = 'F0', stochdepth_rate: float = None,
        ...
        self.stem = Stem_BN(activation=activation)
        ...
        final_conv_channels = 2 * in_channels
        self.final_conv = BNConv2D(in_channels=out_channels, out_channels=final_conv_channels, kernel_size=1)
        self.pool = nn.AvgPool2d(1)
...
class Stem_BN(nn.Module):
    def __init__(self, activation: str = 'gelu'):
        super().__init__()

        self.activation = activations_dict[activation]
        self.conv0 = BNConv2D(in_channels=3, out_channels=16, kernel_size=3, stride=2)
        self.conv1 = BNConv2D(in_channels=16, out_channels=32, kernel_size=3, stride=1)
        self.conv2 = BNConv2D(in_channels=32, out_channels=64, kernel_size=3, stride=1)
        self.conv3 = BNConv2D(in_channels=64, out_channels=128, kernel_size=3, stride=2)
...
class NFBlock_BN(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, expansion: float = 0.5,
        ...
        self.conv0 = BNConv2D(in_channels=self.in_channels, out_channels=self.width, kernel_size=1)
        self.conv1 = BNConv2D(in_channels=self.width, out_channels=self.width, kernel_size=3, stride=stride, padding=1,
                              groups=self.groups)
        self.conv1b = BNConv2D(in_channels=self.width, out_channels=self.width, kernel_size=3, stride=1, padding=1,
                               groups=self.groups)
        self.conv2 = BNConv2D(in_channels=self.width, out_channels=self.out_channels, kernel_size=1)
        ...
        if self.use_projection:
            if stride > 1:
                self.shortcut_avg_pool = nn.AvgPool2d(kernel_size=2, stride=2,
                                                      padding=0 if self.in_channels == 1536 else 1)
            self.conv_shortcut = BNConv2D(self.in_channels, self.out_channels, kernel_size=1)
