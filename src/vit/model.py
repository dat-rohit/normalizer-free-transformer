...
class WSLinear(nn.Linear):
    ...
    def forward(self, input):
        return F.linear(input, self.standardized_weights(), self.bias)


class WSFeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, dropout=0.):
        super().__init__()
        self.net = nn.Sequential(
            WSLinear(dim, hidden_dim),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, dim),
            nn.Dropout(dropout)
...
class WSAttention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.):
        ...
        self.attend = nn.Softmax(dim=-1)
        self.to_qkv = WSLinear(dim, inner_dim * 3, bias=False)
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout=0., normalization='Pre'):
        ...
        elif normalization == 'WS':
            for _ in range(depth):
                self.layers.append(nn.ModuleList([
                    NoNorm(WSAttention(dim, heads=heads, dim_head=dim_head, dropout=dropout)),
                    NoNorm(WSFeedForward(dim, mlp_dim, dropout=dropout))
                ]))
...
class ViTPostNorm(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool='cls', channels=3,
        ...
        self.transformer = Transformer(dim, depth, heads, dim_head, mlp_dim, dropout, normalization='Post')
...
class ViTWithoutNorm(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool='cls', channels=3,
                 dim_head=64, dropout=0., emb_dropout=0.):
        super().__init__()
        image_height, image_width = pair(image_size)
        ...
        self.transformer = Transformer(dim, depth, heads, dim_head, mlp_dim, dropout, normalization='No')
...
class ViTWS(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool='cls', channels=3,
        ...
        self.transformer = Transformer(dim, depth, heads, dim_head, mlp_dim, dropout, normalization='WS')
